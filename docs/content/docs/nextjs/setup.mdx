...
## Cache Revalidation Setup (Optional)
...
### 2. Using the Revalidation Endpoint

To revalidate metadata for a specific path:

```bash
curl -X POST http://localhost:3000/api/generate-metadata/revalidate \
  -H "Authorization: Bearer your_revalidate_secret_here" \
  -H "Content-Type: application/json" \
  -d '{"path": "/blog/my-post"}'
```

To clear all cached metadata:
...
Successful revalidation returns:
